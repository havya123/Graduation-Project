...
class AppStore extends GetxController {
  ...
  Future<AppStore> init() async {
    ...
    if (userSaved.isNotEmpty) {
      Driver? driver = await DriverRepo().getUser(userSaved);
      if (driver == null) {
        return this;
      } else {
        updateUser(driver);
    ...
    if (AppServices.to.getString(MyKey.onDelivery).isNotEmpty) {
      //AppServices.to.setString(MyKey.onDelivery, jsonEncode(false));
  ...
  Future<void> saveUser(String uid) async {
    ...
    if (driver != null) {
      ...
      await AppServices.to.setString(MyKey.user, uid);
    }
  }
